...
def list_connection_requests(db: Session = Depends(get_db)):
    """List connection requests with node names."""
    rows = db.query(ConnectionRequest).order_by(ConnectionRequest.requested_at.desc()).all()
...
def list_connections(db: Session = Depends(get_db)):
    """List all connections with node names."""
    rows = db.query(Connection).order_by(Connection.created_at.desc()).all()
