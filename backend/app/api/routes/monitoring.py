...
async def get_stats_detailed(db: Session = Depends(get_db)):
    ...
    async def fetch_one(n: Node) -> NodeStatsSchema:
        wg = await fetch_wg_status(n.agent_url)
...
def list_logs(event_type: str | None = None, db: Session = Depends(get_db)):
    """List audit logs; optional filter by event_type (e.g. JOIN_APPROVED)."""
